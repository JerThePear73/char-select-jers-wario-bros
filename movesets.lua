...
local SOUND_HUMBLE_SH_BASH = audio_sample_load("JW_SOUND_BASH.ogg")
local SOUND_HUMBLE_LEAP = audio_sample_load("JW_SOUND_JUMP.ogg")
local SOUND_HUMBLE_CORKSCREW = audio_sample_load("JW_SOUND_CORKSCREW.ogg")

local PARTICLE_TIMER = 10
local SH_BASH_MIN = 18
...
function particle_ring_loop(o)
    ...
    if m.action == ACT_SH_BASH or m.action == ACT_SH_BASH_JUMP then
...
local function act_sh_bash(m)
    ...
    elseif (m.controller.buttonDown & B_BUTTON == 0) or ((is_game_paused() or _G.charSelect.is_menu_open()) and m.playerIndex == 0) then
        if m.prevAction == ACT_SH_BASH_JUMP then
            set_mario_action(m, ACT_BRAKING, 0)
        elseif m.actionTimer > SH_BASH_MIN then
            set_mario_action(m, ACT_BRAKING, 0)
        end
...
local function act_humble_gp_cancel(m)
    ...
    local stepResult = common_air_action_step(m, ACT_BUTT_SLIDE_AIR, MARIO_ANIM_START_GROUND_POUND, AIR_STEP_CHECK_LEDGE_GRAB)
    if stepResult == AIR_STEP_HIT_WALL then
        m.particleFlags = m.particleFlags | PARTICLE_VERTICAL_STAR
        set_mario_action(m, ACT_BACKWARD_AIR_KB, 0)
    end

    m.marioObj.header.gfx.scale.y = 2
...
local function wario_update(m)
    ...
    if (m.playerIndex == 0 or is_player_active(m) ~= 0) and m.marioObj.header.gfx.node.flags & GRAPH_RENDER_ACTIVE ~= 0 then
        if ((m.action == ACT_SH_BASH or m.action == ACT_SH_BASH_JUMP) and m.forwardVel >= 50) or (m.action == ACT_CORKSCREW and m.vel.y > 20) then
...
local function wario_set_action(m)
    ...
    --end
        ...
        -- ring particles
    if (m.playerIndex == 0 or is_player_active(m) ~= 0) and m.marioObj.header.gfx.node.flags & GRAPH_RENDER_ACTIVE ~= 0 then
        if m.action == ACT_SH_BASH and m.prevAction ~= ACT_SH_BASH_JUMP then
